<?php
  ...
  if(isset($_POST['cart_menu_id'])){
    if(isset($_SESSION['user'])){
      $check_sql = "SELECT id FROM meal_order WHERE menu_id = '$_POST[cart_menu_id]' AND customer_id = '$_SESSION[user]'"; 
      $check_req = mysqli_query($connection, $check_sql);
      if(mysqli_num_rows($check_req) < 1){
        $add_fav_sql = "INSERT INTO meal_order (menu_id, customer_id, location) VALUES ('$_POST[cart_menu_id]', '$_SESSION[user]', '$_SESSION[location]')";
        $add_fav_req = mysqli_query($connection, $add_fav_sql);
      }
    } 
    else{
